...
<body>
  ...
  <script>
    const nodes = ["十六万", "三万八", "小四", "大傻", "二傻", "小五"];
...
def _run(host, user, cmd):
    if host == "127.0.0.1":
        ...
        r = subprocess.run(["ssh", "-o", "ConnectTimeout=5", "-o", "StrictHostKeyChecking=no",
            f"{user}@{host}", cmd], capture_output=True, text=True, timeout=8)
    return r.stdout

def _is_mac(host, user):
    try:
        return "Darwin" in _run(host, user, "uname")
    except: return False

def _node_cpu(host, user):
    try:
        if _is_mac(host, user):
            out = _run(host, user, "top -l 1 -n 0 2>/dev/null | grep 'CPU usage'")
            m = re.search(r'(\d+\.?\d*)% idle', out)
            if m: return f"{100 - float(m.group(1)):.1f}%"
        else:
            out = _run(host, user, "top -bn1 2>/dev/null | grep 'Cpu(s)' || mpstat 2>/dev/null | tail -1")
            m = re.search(r'(\d+\.?\d*)% idle', out)
            if m: return f"{100 - float(m.group(1)):.1f}%"
    except: pass
    return "--"

def _node_mem(host, user):
    try:
        if _is_mac(host, user):
            out = _run(host, user, "vm_stat | head -10")
            pages = re.findall(r'(\d+)\.', out)
            if len(pages) >= 2:
                active, free_p = int(pages[0]), int(pages[1])
                total = active + free_p
                if total > 0: return f"{active*100//total}%"
        else:
            out = _run(host, user, "free | grep Mem")
            m = re.search(r' (\d+) +(\d+)', out)
            if m: return f"{int(m.group(2))*100//int(m.group(1))}%"
    except: pass
    return "--"

def _node_disk(host, user):
    try:
        out = _run(host, user, "df -h / | tail -1")
        parts = out.split()
        if len(parts) >= 5: return parts[4]
    except: pass
    return "--"

def _node_uptime(host, user):
    try:
        out = _run(host, user, "uptime -p 2>/dev/null || uptime")
        return out.strip()[:50] or "--"
    except: return "--"

                "online": True,
            }
        except Exception as e:
            results[name] = {"cpu": "--", "mem": "--", "disk": "--", "uptime": "--", "online": False, "error": str(e)[:30]}
    try:
        results["ECS 香港"]["deepseek"] = round(_read_deepseek_balance(), 2)
    except:
        results["ECS 香港"]["deepseek"] = 0
