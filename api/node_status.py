...
def _r(host, user, cmd, is_local):
    try:
        if is_local:
            r = subprocess.run(["bash", "-c", cmd], capture_output=True, text=True, timeout=4)
        else:
            r = subprocess.run(["ssh", "-i", SSH_KEY, "-o", "ConnectTimeout=3",
                "-o", "StrictHostKeyChecking=no", f"{user}@{host}", "bash", "-c", cmd],
...
def _coll(name, host, user, is_local):
    try:
        ...
        if mac:
            o = _r(host, user, "top -l 1 -n 0 2>/dev/null | grep 'CPU usage'", is_local)
            m = re.search(r'(\d+\.?\d*)% idle', o)
            if m: cpu = f"{100 - float(m.group(1)):.1f}%"
            o = _r(host, user, "vm_stat | head -8", is_local)
...
async def ecs_only():
    return (await api_nodes_status())["nodes"].get("ECS 香港", {})
